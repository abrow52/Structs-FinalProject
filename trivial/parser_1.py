...
#added operators into arithmetic_factor(**), relational_expression(in, not in), assignmnet_expression(+=, -=, *=, **=) 
grammar = """
    ...
    arithmetic_factor = complex_expression [ "**" arithmetic_factor ]
    arithmetic_term = arithmetic_factor { ("*" | "/") arithmetic_factor }
    arithmetic_expression = arithmetic_term { ("+" | "-") arithmetic_term }
    relational_expression = arithmetic_expression { ("<" | ">" | "<=" | ">=" | "==" | "!=" | "in" | "not in") arithmetic_expression }
...
#modified to work with power precedence
def parse_arithmetic_factor(tokens):
    ...
    if tokens[0]["tag"] == "**":
        right, tokens = parse_arithmetic_factor(tokens[1:])
...
def test_parse_arithmetic_factor():
    ...
    ast, _ = parse_arithmetic_factor(tokenize("2**4"))
    assert ast == {
        "tag": "**",
        "left": {"tag": "number", "value": 2},
    ...
    ast, _ = parse_arithmetic_factor(tokenize("2**3**2"))
    assert ast == {
        "tag": "**",
        "left": {"tag": "number", "value": 2},
        "right": {
...
def parse_relational_expression(tokens):
    ...
    while tokens[0]["tag"] in ["<", ">", "<=", ">=", "==", "!=", "in", "not in"]:
        tag = tokens[0]["tag"]
...
def parse_assignment_expression(tokens):
    ...
    if tokens[0]["tag"] in [ "=", "+=", "-=", "*=", "**=" ]:
        ...
        if op == "=":
            ...
            arith_op = op[:-1]
            value = {"tag": arith_op, "left": left.copy(), "right": right}

        return {"tag": "assign", "target": left, "value": value}, tokens
...
def test_parse_assignment_expression():
    ...
    ast, tokens = parse_assignment_expression(tokenize("x -= 8"))
    assert ast == {
        "tag": "assign",
        "target": {"tag": "identifier", "value": "x"},
        "value": {
            "tag": "-", 
            "left": {"tag": "identifier", "value": "x"}, 
            "right": {"tag": "number", "value": 8}
            },
    }

    ast, tokens = parse_assignment_expression(tokenize("x *= 10"))
    assert ast == {
        "tag": "assign",
        "target": {"tag": "identifier", "value": "x"},
        "value": {
